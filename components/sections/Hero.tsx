...
export default function Hero() {
  return (
    ...
    >
      {/* Aura animated background — scoped to hero only */}
      <AuraBackground />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-4 max-w-6xl mx-auto">
        {/* Badge */}
        <div className="mb-8 opacity-0 animate-[fadeInUp_0.8s_ease-out_1.55s_forwards] flex justify-center">
          <AnimatedBadge
        ...
        <h1 className="font-bold leading-[0.95] tracking-tighter mb-6 mix-blend-screen relative hero-title" style={{ fontFamily: 'var(--font-hero)' }}>
          <span className="block text-[11vw] md:text-[5.5vw]">
            <span style={{ display: 'inline-block', overflow: 'hidden', verticalAlign: 'bottom', marginRight: '0.2em' }}>
              <span className="block bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-zinc-500" style={{ animation: 'wordReveal 1.1s cubic-bezier(0.16,1,0.3,1) 0.65s both' }}>CUSTOM</span>
            </span>
            <span style={{ display: 'inline-block', overflow: 'hidden', verticalAlign: 'bottom' }}>
              <span className="block bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-zinc-500" style={{ animation: 'wordReveal 1.1s cubic-bezier(0.16,1,0.3,1) 0.83s both' }}>SOFTWARE</span>
            </span>
          </span>
          <span className="block text-[11vw] md:text-[5.5vw]">
            <span style={{ display: 'inline-block', overflow: 'hidden', verticalAlign: 'bottom' }}>
              <span className="block bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-zinc-500" style={{ animation: 'wordReveal 1.1s cubic-bezier(0.16,1,0.3,1) 1.01s both' }}>SOLUTIONS</span>
            </span>
          </span>
        </h1>
          ...
          <p className="text-xs md:text-sm text-zinc-400 text-center leading-relaxed" style={{ fontFamily: 'var(--font-hero)' }}>
            {['Custom', 'websites,', 'apps,', 'and', 'software', 'made', 'to', 'fit', 'your', 'vision.'].map((word, i) => (
              <span key={i} style={{ display: 'inline-block', overflow: 'hidden', verticalAlign: 'bottom', marginRight: '0.28em' }}>
                <span className="block" style={{ animation: `wordReveal 1s cubic-bezier(0.16,1,0.3,1) ${0.65 + i * 0.07}s both` }}>
                  {word}
                </span>
              </span>
            ))}
            ...
            <span className="text-zinc-500">
              {['From', 'a', 'landing', 'page', 'to', 'a', 'full', 'SaaS', 'platform,', 'we', 'build', 'it', 'right.'].map((word, i) => (
                <span key={i} style={{ display: 'inline-block', overflow: 'hidden', verticalAlign: 'bottom', marginRight: '0.28em' }}>
                  <span className="block" style={{ animation: `wordReveal 1s cubic-bezier(0.16,1,0.3,1) ${1.21 + i * 0.05}s both` }}>
                    {word}
                  </span>
                </span>
              ))}
